...
function newTicket(ticket) {
  if (helpTicketList.length === 15) {
    //deletes old ticket and adds new one
    helpTicketList = helpTicketList.filter(
      (ticket) => ticket != helpTicketList[14]
    );
...
new newPost(
  "Welcome🥳🥳.",
  "Welcome to my first live project site. Feel free to share a post!",
  "4241"
);
...
function getId() {
  //gets random 6 digit id for post
  var idList = [];
  var id = "";
  var alphabet = [
    "a",
...
function newComment(comment, postId) {
  //adds new comment to post by the post's id
...
app.post("/commentLike/:postId/:id", (req, res) => {
  ...
  var commentIndexNumber = post.comments.findIndex(
    (comment) => comment.id == commentId
  ); //gets index of comment inside the post
